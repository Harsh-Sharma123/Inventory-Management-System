...
class category:
    def __init__(self, root):
        ...
        cat_frame.place(x=700, y=100, width=300, height=120)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get() == "":
                ...
                row = cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","This Category is already assigned, try different one.")
                else:
    ...
    def delete(self):
        ...
        try:
            if self.var_cat_id.get()=="":
                messagebox.showerror("Error", "Select or enter a category name!", parent=self.root)
            else:
