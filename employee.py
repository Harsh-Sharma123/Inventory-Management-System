...
class employeeClass:
    def __init__(self, root):
        ...
        btn_add = Button(self.root, text="Save", command=self.add, font=("oudy old style", 15), bg="#2196f3", fg="white", cursor='hand2').place(x=500, y=305, width=110, height=28)
        btn_update = Button(self.root, text="Update", command=self.update, font=("oudy old style", 15), bg="#4caf50", fg="white", cursor='hand2').place(x=620, y=305, width=110, height=28)
        btn_delete = Button(self.root, text="Delete", font=("oudy old style", 15), command=self.delete, bg="#f44336", fg="white", cursor='hand2').place(x=740, y=305, width=110, height=28)
        btn_clear = Button(self.root, text="Clear", command=self.clear, font=("oudy old style", 15), bg="#607f8b", fg="white", cursor='hand2').place(x=860, y=305, width=110, height=28)
...
# =================================================================================
    def add(self):
        ...
        try:
            if self.var_emp_id.get() == "":
                ...
                row = cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","This Employee Id is already assigned, try different one.")
                else:
    ...
    def update(self):
        ...
        try:
            if self.var_emp_id.get()=="":
                messagebox.showerror("Error", "Employee ID is empty!")
            else:
                cur.execute("Select * from employee where eid=?", (self.var_emp_id.get(),))
                row = cur.fetchone()
                if row==None:
